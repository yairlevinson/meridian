...
export type CoreRadarManager = TrackingRadarSource & {
  destroy: () => void
}

export interface CoreRuntimeOptions<TRadar extends CoreRadarManager> {
  userDataPath: string
  udpPort: number
  tcpLinks?: string
  createRadarManager: (settingsManager: SettingsManager) => TRadar
}

export interface CoreRuntime<TRadar extends CoreRadarManager> {
...
export async function createCoreRuntime<TRadar extends CoreRadarManager>(
  options: CoreRuntimeOptions<TRadar>
): Promise<CoreRuntime<TRadar>> {
  ...
  const radarManager = options.createRadarManager(settingsManager)
